...
router.get("/rand20", (req, res) => {
  ...
  sql +=
    " inner join tb_typeproduct on tb_product.typepro = tb_typeproduct.id_typepro";
  sql += " WHERE id_mtype = 1";
  sql += " ORDER BY date_check DESC,rand() LIMIT 0, 20";
  conn.query(sql, (err, result) => {
    res.send(result);
  });
});
...
router.get("/checkB1_v2", (req, res) => {
  ...
  function checkDateBefore() {
    ...
    // sql += " ORDER BY date_check DESC";
    sql += " ORDER BY name_pro";
    conn.query(sql, [build,arrDate[1].date_check], (err, resp) => {
      // console.log(resp);
      beforePrice = resp;
      mapChangePriceNow();
    });
  ...
  function mapDiffPrice() {
    ...
    let diffmap = marge_nowPrice.map((item) => {
      ...
      return {
        ...
        id_mtype: item.id_mtype,
        date_check: item.date_check,
        date_check_th: moment(item.date_check)
          .add(543, "years")
          .format("DD-MM-YYYY"),
        unitname: item.unitname,
        diff_price: val_diff,
...
router.get("/searchdate", (req, res) => {
  ...
  function fnGetDate() {
    ...
    var sqlDateChk =
      // "SELECT date_check FROM tb_priceproduct ORDER BY id_pricepro DESC LIMIT 0,2";
      "SELECT date_check FROM tb_priceproduct WHERE date_check BETWEEN '2017-01-01' AND ? ORDER BY id_pricepro DESC LIMIT 0,2"
    conn.query(sqlDateChk,[g_date] ,(err, resp) => {
      if (err) throw console.log(err);
      console.log(resp);
      arrDate = resp;
      fnSearchdate();
  ...
  // SELECT date_check FROM tb_priceproduct WHERE date_check BETWEEN '2020-10-5' AND '2016-01-01' ORDER BY id_pricepro DESC LIMIT 0,2

  function fnSearchdate() {
    let g_date = req.query.g_date;
    let g_build = req.query.build;
    ...
    // sql += "WHERE date_check = ? and mt.bud = ? ";
    sql += "WHERE date_check = ? and ty.id_mtype = ? ";
    sql += "group by id_prod order by name_pro";
    conn.query(sql, [g_date, g_build], (err, resp) => {
      if (err) throw console.log(err);
      //res.send(resp);
      arrData_now = resp;
      fnSearchdate_before();
  ...
  function fnSearchdate_before() {
    ...
    sql += "WHERE date_check = ? and ty.id_mtype = ? ";
    sql += "group by id_prod order by name_pro";
    conn.query(sql, [arrDate[1].date_check, g_build], (err, resp) => {
      if (err) throw console.log(err);
      //res.send(resp);
      arrData_before = resp;
      mapChangePriceNow();
  ...
  // เปลี่ยนชื่อ ราคา
  function mapChangePriceNow() {
    let newPrice = arrData_now.map((item) => {
      return {
        id_prod: item.id_prod,
        name_pro: item.name_pro,
        price_pro_now: item.price_pro,
   ...
   function margePrice() {
    let arrMarge = [];
    for (let i = 0; i < arrData_now.length; i++) {
      marge = { ...arrData_before[i], ...arrData_now[i] };
      arrMarge.push(marge);
    }
    // console.log(arrMarge);
    marge_nowPrice = arrMarge;
  ...
  function sendData() {
    // res.send(arrData_before);
    // res.send(arrData_now);
    res.send(g_dataPrice);
  }
});
